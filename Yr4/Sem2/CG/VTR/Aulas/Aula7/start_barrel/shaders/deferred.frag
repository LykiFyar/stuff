...
uniform sampler2D diffuseY, diffuseB, diffuseG, diffuseR, diffuseBl, rust, specularMap, normalMap,
...
void main() {
    ...
    vec3 t = normalize(texture(tangent, tc).xyz * 2.0 - 1.0);
    ...
    vec3 nMap = normalize(texture(normalMap, tc_obj).xyz * 2.0 - 1.0);
    ...
    vec4 color;
    if (cr == 0) color = texture(diffuseY, tc_obj);
    else if (cr == 1) color = texture(diffuseB, tc_obj);
    else if (cr == 2) color = texture(diffuseG, tc_obj);
    else if (cr == 3) color = texture(diffuseR, tc_obj);
    else color = texture(diffuseBl, tc_obj);

    vec4 texRust = texture(rust, tc_obj);
    float specMap = texture(specularMap, tc_obj).r;

    float noise = clamp(pow((perlinNoise(p.xyz) + 0.5) * specMap * 1.5, 4), 0.0, 1.0);
    ...
    if (intensity > 0) {
        ...
        float specInt = max(0.0, dot(h, nMap));
