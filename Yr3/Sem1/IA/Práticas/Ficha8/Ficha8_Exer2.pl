...
%Leitores do sexo feminino
feminino(L) :- findall((X,Y),leitores(X,Y,f),L).

qleitoresfem(N) :- feminino(S), length(S,N).


%Leitores de uma cidade
leitorcidade(C,R) :-
    leitores(X,R,_),
    requisicoes(_,X,Y,_),
    livros(Y,_,Z),
...
todosleitorescidade(C,L) :- findall(X,leitorcidade(C,X),L).

%Quais livros requisitados e leitores numa cidade

livroleitorcidade(C,S) :- findall((N,Leitor),(requisicoes(_,IL,Livro,_),leitores(IL, Leitor, _),livros(Livro,N,B),biblioteca(B,_,C)),S).

%Lista de livros e data de requisições pedidos em (ano)
requisitadoemano(A,L) :- findall((Livro,data(A,M,D)),(livros(IL,Livro,_),requisicoes(_,_,IL,data(A,M,D))),L).

%livro requisitado em ano
livrorequisitadoemano(Livro,A,L) :- findall((Livro,data(A,M,D)),(livros(IL,Livro,_),requisicoes(_,_,IL,data(A,M,D))),L).
