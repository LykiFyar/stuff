...
void prepareCylinder(float altura, float raio, int lados) {
	...
	for (int i = 0; i < lados; ++i) {
		...
		t[vertex * 2 + 0] = 0.4375 + cos(i * delta) * 0.1875;
		...
		t[vertex * 2 + 0] = 0.4375 + cos((i+1) * delta) * 0.1875;
		...
		v[vertex*3 + 1] = -altura/2.0f;
		v[vertex*3 + 2] = raio * cos( (i+1) * delta);
		t[vertex * 2 + 0] = 0.8125 + cos((i+1) * delta) * 0.1875;
		t[vertex * 2 + 1] = 0.1875 + sin((i+1) * delta) * 0.1875;
		...
		v[vertex*3 + 1] = -altura/2.0f;
		v[vertex*3 + 2] = raio * cos( i * delta);
		t[vertex * 2 + 0] = 0.8125 + cos(i * delta) * 0.1875;
		t[vertex * 2 + 1] = 0.1875 + sin(i * delta) * 0.1875;
...
void renderScene(void) {
	...
	int floorID = loadTexture("Concrete.jpg");
	int barrelID = loadTexture("Oil_Drum001h.jpg");
	...
	drawFloor(-5, 5, -5, 5);
	glTranslatef(0, 1, -1);
	glBindTexture(GL_TEXTURE_2D, barrelID);
	drawCylinder();
	glTranslatef(0, 0, 2);
	glBindTexture(GL_TEXTURE_2D, barrelID);
	drawCylinder();
