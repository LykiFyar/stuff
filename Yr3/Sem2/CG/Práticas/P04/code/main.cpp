...
void prepareData(vector<float> p) {
	//????realloc(&vertices, p.size() * sizeof(float));
	// criar o VBO 
...
/*-----------------------------------------------------------------------------------
	Draw Cylinder for VBO (make vector)
...
vector<float> cylinderVector(float radius, float height, int sides) {
...
//  OpenGL settings
	...
	prepareData(cylinderVector(1, 2, sides));
	printInfo();
