...
void computeNormal(int i, int j, std::vector<float>& normal) {
	...
	float v1[3] = {0, h(i, j + 1) - h(i, j - 1), 2}; // x = i - i , y = h(i, j+1) - h(i,j-1), z = j+1 - (j-1)
	float v2[3] = { 2, h(i + 1, j) - h(i - 1,j), 0 }; // x = i+1 - (i-1) , y = h(i+1, j) - h(i-1,j), z = j - j
	float res[3] = { 0,0,0 };
...
void prepareTerrain() {
	...
	// copy data from texCoord and normal to OpenGL buffers
	...
	// Enable texCoord and normal buffers
...
void renderTerrain() {
	...
	// give semantic to texcoord and normal buffers
